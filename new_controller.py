...
buttons = {'x':0.,'o':0.,'t':0.,'s':0.,
           'L1':0.,'R1':0.,'L2':0.,'R2':0.,
	   'share':0.,'options':0.,
           'axis1':0.,'axis2':0.,'axis3':0.,'axis4':0.,'Dpad up':0,
           'Dpad down':0, 'Dpad left':0,'Dpad right':0}
...
#             print(event.dict,event.joy,event.button,'RELEASED')
            ...
            for x,(key,val) in enumerate(buttons.items()):
                if x == 14 or x == 15:
                    if hat[1] > 0: buttons['Dpad up'] = hat[1]
                    else: buttons['Dpad up'] = 0
                    if hat[1] < 0: buttons['Dpad down'] = abs(hat[1])
                    else: buttons['Dpad down'] = 0
                if x == 16 or x == 17:
                    if hat[0] > 0: buttons['Dpad right'] = hat[0]
                    else: buttons['Dpad right'] = 0
                    if hat[0] < 0: buttons['Dpad left'] = abs(hat[0])
                    else: buttons['Dpad left'] = 0
